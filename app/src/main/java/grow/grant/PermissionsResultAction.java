...
public abstract class PermissionsResultAction {
    ...
    protected synchronized final boolean onResult(final @NonNull String permission, int result) {
        if (result == PackageManager.PERMISSION_GRANTED) {
            return onResult(permission, PermissionsManager.Permissions.GRANTED);
        } else {
            return onResult(permission, PermissionsManager.Permissions.DENIED);
        }
    ...
    @SuppressWarnings("WeakerAccess")
    @CallSuper
    protected synchronized final boolean onResult(final @NonNull String permission, PermissionsManager.Permissions result) {
        if(!mPermissions.contains(permission))
            return false;

        mPermissions.remove(permission);
        if (result == PermissionsManager.Permissions.GRANTED) {
        ...
        } else if (result == PermissionsManager.Permissions.DENIED) {
            new Handler(mLooper).post(new Runnable() {
                @Override
                public void run() {
                    onDenied(permission);
            ...
            return true;
        } else if (result == PermissionsManager.Permissions.NOT_FOUND) {
